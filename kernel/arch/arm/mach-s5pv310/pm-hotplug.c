...
static ssize_t show_cpu_freq(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	unsigned int cpu0_freq=0,cpu1_freq=0;
	
	mutex_lock(&hotplug_lock);
	
	cpu0_freq = cpufreq_get(0);
	if(cpu_online(1))	cpu1_freq = cpufreq_get(1);
	else 				cpu1_freq = 0;

	mutex_unlock(&hotplug_lock);
	
	return sprintf(buf, "Cpu0 : %u\nCpu1 : %u\n", cpu0_freq, cpu1_freq);
}
static DEVICE_ATTR(cpu_freq, S_IRWXUGO, show_cpu_freq, NULL);
...
static int __init s5pv310_pm_hotplug_device_init(void)
{
	...
	ret = device_create_file(&s5pv310_pm_hotplug_device.dev, &dev_attr_cpu_freq);
