...
static int s5pv310_clk_block(struct clk *clk, int enable)
{
...
{
	return s5p_gatectrl(S5P_CLKGATE_SCLKCAM, clk, enable);
}
...
static int s5pv310_clk_sclkapll_ctrl(struct clk *clk, int enable)
{
	return s5p_gatectrl(S5P_CLKGATE_SCLKCPU, clk, enable);
}
...
// HISEOB : Default Disabled Clocks
...
/*	{ // remove for enable TMU by ydongyol 2011.5.20
		.name		= "tmu_apbif",
		.id		= -1,
		.enable		= s5pv310_clk_ip_perir_ctrl,
		.ctrlbit	= (1 << 17),
...
*/
#if 0
	{
		.name		= "sclkapll",
		.id		= -1,
		.enable		= s5pv310_clk_sclkapll_ctrl,
		.ctrlbit	= (1 << 0),
	},
#endif
	{
...
static struct clk init_clocks[] = {
	...
	/* HISEOB : Additional Enable Clocks */
	{
		.name = "sromc",
		.id = 1,
		.enable = s5pv310_clk_ip_fsys_ctrl,
...
//	clk_set_rate(&clk_sclk_pixel.clk, 2000000);
}
