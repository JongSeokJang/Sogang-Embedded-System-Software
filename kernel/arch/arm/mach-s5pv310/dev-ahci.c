...
static void __iomem *phy_base, *phy_ctrl, *ahci_base;

/* Voodoo. Don't Ask Can't Tell */
int ahci_phy_init(void)
{
	int i;
	u32 val;
...
static int s5pv310_ahci_init(struct device *dev)
{
	struct clk *clk_sata, *clk_sataphy, *clk_sclk_sata;
	u32 val;

	ahci_base = ioremap(S5PV310_PA_AHCI, 0x10000);
	if (!ahci_base) {
		dev_err(dev, "Unable to ioremap AHCI\n");
		return -ENOMEM;
	}
	...
	/* Only 1 Port */
	__raw_writel(1, ahci_base + 0xc/*HOST_PORTS_IMPL*/);

	return ahci_phy_init();
}
