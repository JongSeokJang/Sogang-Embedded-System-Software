...
#else
/* SPI */
static UINT g_nUsedSubchOutBufIdxBits; /* 0 ~ (RTV_MAX_NUM_SUB_CHANNEL_USED-1) */
static UINT g_aSubchOutBufIdx[64];
...
//	RTV_DBGMSG1("\n[rtvCifParser_Parsing] msc_total: %d\n", nTsLen);
	...
	do
	{
		...
		{
			RTV_DBGMSG0("[rtvCIFDEC_Decode] Invalid ch length. May be broken...\n"); 
			return;
		}
		...
		/* Verify a Header */
		if(subch_size > MAX_SUBCH_SIZE_PER_CIF) 
		{
			RTV_DBGMSG0("[rtvCIFDEC_Decode] Invalid sub ch SIZE.\n"); 
			return;
		}
		...
		if(subch_size != 0) // 
		{
			...
			{
				RTV_DBGMSG0("[rtvCIFDEC_Decode] Invalid sub ch ID.\n"); 
				return;
			}
		}
		
		nOutBufIdx = g_aSubchOutBufIdx[subch_id]; // Out buffer index determined when add sub channel.
      
		ptDecInfo->msc_subch_id[nOutBufIdx] = subch_id;		

		CIF_DATA_COPY(dest_msc_buf_ptr[nOutBufIdx], data_ptr, subch_size);		
		
		dest_msc_buf_ptr[nOutBufIdx]    += subch_size;
		ptDecInfo->msc_size[nOutBufIdx] += subch_size;				
...
void rtvCIFDEC_DeleteSubChannelID(UINT nSubChID)
{
	...
	g_nUsedSubchOutBufIdxBits &= ~(1<<g_aSubchOutBufIdx[nSubChID]);
#endif
}
...
void rtvCIFDEC_AddSubChannelID(UINT nSubChID)
{
	...
	for(i=0; i<RTV_MAX_NUM_SUB_CHANNEL_USED; i++) 
	{
		if((g_nUsedSubchOutBufIdxBits & (1<<i)) == 0) 		
		{
			nOutBufIdx = i;
			break;
		}
	...
	g_aSubchOutBufIdx[nSubChID] = nOutBufIdx;
	g_nUsedSubchOutBufIdxBits |= (1<<nOutBufIdx);
#endif
}
