...
static void fm_InitDemod(void)
{
	...
	if(g_aeRtvAdcClkFreqType[RaonTvChipIdx] == RTV_ADC_CLK_FREQ_8_MHz)
	{
	...
	else if(g_aeRtvAdcClkFreqType[RaonTvChipIdx] == RTV_ADC_CLK_FREQ_8_192_MHz)
	{
...
INT rtvFM_Initialize(E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType, UINT nThresholdSize)
{
	...
	/* If the other TV mode included, the FM must use the master chip. So, we selects the master. */
	RTV_MASTER_CHIP_SEL;
