...
#if !defined(RTV_CIF_MODE_ENABLED)
	...
	for(;;)
	{
		...
		{			
			...
			switch( tv_mode ) /* 0: 1seg, 1: T_DMB, 2: FM */
			{
				...
				case 2 :
					fwrite(tsp->msc_buf, sizeof(char), tsp->len, fd_fm_ts);
					break;
...
static IOCTL_FM_SCAN_INFO fm_result_scan_info;
static void test_FM(void)
{
	int key, ret;
	E_RTV_ADC_CLK_FREQ_TYPE adc_clk_type; 
	unsigned int ch_freq_hz;
	IOCTL_FM_LOCK_STATUS_INFO fm_lock_status;
	int i;
	char devname[32];
...
#ifdef  _TS_FILE_DUMP_ENABLE
	if((fd_fm_ts=fopen("./fm.pcm", "wb")) == NULL)
	{
		printf("fm.pcm file open error\n");
	}
#endif	
	...
	{
		printf("===============================================\n");
		printf("\t0: FM Power ON\n");
		printf("\t1: FM Power OFF\n");
		printf("\t2: FM Scan freq\n");
		printf("\t3: FM Set Channel\n");
		printf("\t4: FM Get Lockstatus\n");
		...
		switch( key )
		{
			case '0':
				...
				do
				{
					printf("Input ADC clock type (0[8 MHz] or 1[8.192 MHz]):" );
					scanf("%d", &adc_clk_type);
					CLEAR_STDIN;					
				} while( (adc_clk_type != 0) && (adc_clk_type != 1));
				printf("[FM ADC clock] : %s\n", adc_clk_type==0 ? "8 MHz" : "8.192 MHz");
				
				if(ioctl(fd_dmb_dev, IOCTL_FM_POWER_ON, &adc_clk_type) != 0)
				{
					printf("IOCTL_FM_POWER_ON error");
				}	
				break;

			case '1':\
				printf("[FM Power OFF]\n");
				if(ioctl(fd_dmb_dev, IOCTL_FM_POWER_OFF) != 0)
				{
					printf("IOCTL_FM_POWER_OFF error");
				}
				break;
		
			case '2':
				fm_result_scan_info.start_freq = 76000;
				fm_result_scan_info.end_freq = 108000;
				fm_result_scan_info.num_ch_buf = MAX_NUM_FM_EXIST_CHANNEL;

				printf("[FM Scan] start_freq(%u) ~ end_freq(%u) \n", fm_result_scan_info.start_freq, fm_result_scan_info.end_freq);	

				if(ioctl(fd_dmb_dev, IOCTL_FM_SCAN_FREQ, &fm_result_scan_info) < 0)
				{
					printf("scan failed\n");
				}		
					
				for(i=0; i<fm_result_scan_info.num_deteced_ch; i++)
				{
					printf("[FM Scan] Detected freq (%u)\n", fm_result_scan_info.ch_buf[i]);
				}				
				break;
			...
			case '3':
				while(1)
				{
					printf("Input Channel freq(ex. 107700):");
					scanf("%u", &ch_freq_hz);
					CLEAR_STDIN;

					if((ch_freq_hz<RTV_FM_CH_MIN_FREQ_KHz) || (ch_freq_hz>RTV_FM_CH_MAX_FREQ_KHz))
					{
						printf("[FM Set Freq] freq must be in the (%dKHz ~ %dKHz)", RTV_FM_CH_MIN_FREQ_KHz, RTV_FM_CH_MAX_FREQ_KHz);
						continue;
					}
					
					if((ch_freq_hz % RTV_FM_CH_STEP_FREQ_KHz) != 0)
					{
						printf("[FM Set Freq] The step of freq must be %dKHz", RTV_FM_CH_STEP_FREQ_KHz);
						continue;
					}	
					
					printf("[FM Set Freq] (ch: %u)\n", ch_freq_hz);
					break;
				}
				...
				if(ioctl(fd_dmb_dev, IOCTL_FM_SET_FREQ, &ch_freq_hz) < 0)
				{
					// scan fail
					printf("IOCTL_FM_SET_FREQ failed\n");
				}								
				printf("\n");
				break;

			case '4':
				printf("[FM Get Lockstatus]\n");
				
				if(ioctl(fd_dmb_dev, IOCTL_FM_GET_LOCK_STATUS, &fm_lock_status) < 0)
				{
					printf("IOCTL_FM_GET_LOCK_STATUS failed\n");
				}

				printf("fm_lock_status.val = %u\n", fm_lock_status.val);
				printf("fm_lock_status.cnt = %u\n", fm_lock_status.cnt);
