...
#if defined(MALI_DVFS_TRACKING)
#include <linux/proc_fs.h>

static char mali_dvfs_name[] = "mali_clk";
static struct proc_dir_entry *mali_entry;
static int mali_dvfs_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data);
#endif
...
#if defined(MALI_DVFS_TRACKING)
	if (mali_entry == NULL){
		mali_entry = create_proc_entry(mali_dvfs_name, 0444, NULL);
		if (mali_entry != NULL) {
			mali_entry->read_proc = mali_dvfs_proc_read;
		}
	}
#endif
    return MALI_TRUE;
...
#if defined(MALI_DVFS_TRACKING)
	if (mali_entry != NULL){
	    remove_proc_entry(mali_dvfs_name, NULL);
	    mali_entry = NULL;
	}
#endif
...
{
	int len;
	unsigned int curStatus = get_mali_dvfs_staus();

	len = sprintf(page, "%d\n", mali_dvfs[curStatus].clock);
//	len = sprintf(page, " [MALI] step = %d\n [MALI] clock = %d MHz\n [MALI] vdd_g3d = %d\n", curStatus,mali_dvfs[curStatus].clock, mali_dvfs[curStatus].vol);
	return len;
}
#endif
