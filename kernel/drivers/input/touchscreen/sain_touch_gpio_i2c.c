...
#define	TOUCH_WR_ADDR			0x40//0x0A
#define	TOUCH_RD_ADDR				0x41//0x0B
...
//[*]----------------------------------------------------------------------------------------------[*]
static	void 		gpio_i2c_short_write		(unsigned short wdata)
{
	unsigned char	cnt, mask;
...
int 				achro4210_touch_write		(unsigned char addr, unsigned char *wdata, unsigned char wsize)
{
	...
	if((ack = gpio_i2c_chk_ack()))	{
		printk(KERN_INFO "[%s:%d] [write address] : no ack\n", __FUNCTION__, __LINE__);
	}
...
write_stop:
	...
	if(ack)
		printk("%s : error ack(%d)\n", __FUNCTION__, ack);
	return	ack;
}

//[*]----------------------------------------------------------------------------------------------[*]
int achro4210_touch_short_write	(unsigned char addr, unsigned short *wdata, unsigned char wsize)
{
	unsigned char cnt, ack;
	...
	if(wsize)	{
		for(cnt = 0; cnt < wsize; cnt++)	{
			gpio_i2c_short_write(wdata[cnt]);
			if((ack = gpio_i2c_chk_ack()))	{
				goto	write_stop;
			}
		}
...
//[*]----------------------------------------------------------------------------------------------[*]
int 				achro4210_touch_array_read		(unsigned char addr, unsigned char *rdata, unsigned char rsize)
{
