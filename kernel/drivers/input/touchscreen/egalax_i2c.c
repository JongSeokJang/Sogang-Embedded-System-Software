...
#define RELEASE_DATE "2011/07/12"
...
//	int gpio = irq_to_gpio(irq);
	int gpio = S5PV310_GPX1(0); //jhkang
...
//printk("[Egalax_i2c]%s\n", __FUNCTION__);
	...
	if( cdev == NULL )
	{
        	printk("[Egalax_i2c] No such char device node \n");
		return -ENODEV;
	}
...
//	printk("[Egalax_i2c] CDev open done!\n");
...
//	printk("[Egalax_i2c] CDev release done!\n");
...
//	printk("[Egalax_i2c] \"%s\" reading fifo data\n", current->comm);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,33)
...
//printk("[Egalax_i2c]%s\n", __FUNCTION__);
	...
	up(&cdev->sem);
//	printk("[Egalax_i2c] I2C writing %zu bytes.\n", count);
	kfree(tmp);
...
//	printk("[Egalax_i2c] \"%s\" call proc_read\n", current->comm);
...
//	printk("[Egalax_i2c] \"%s\" call proc_write\n", current->comm);
	...
	{
		printk("[Egalax_i2c] proc_write faied at copy_from_user\n");
		return -EFAULT;
	}
...
//	printk("[Egalax_i2c] Switch Debug Level to 0x%08X\n", DbgLevel);
...
//	printk("[Egalax_i2c] Handle device ioctl command\n");
	switch (cmd)
	{
...
//printk("[Egalax_i2c]%s\n", __FUNCTION__);
	...
	{
		printk("[Egalax_i2c] Get I2C Point data error [%02X][%02X][%02X][%02X][%02X][%02X]\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
		return;
	}

//	PointBuf[ContactID].X = (1280*X)/2047;
//	PointBuf[ContactID].Y = (800*(2047-Y))/2047;

	PointBuf[ContactID].X = (800*(2047-Y))/2047;
	PointBuf[ContactID].Y = (1280*X)/2047;
...
//	printk("[Egalax_i2c] Get Point[%d] Update: Status=%d X=%d Y=%d DownCnt=%d\n", ContactID, Status, X, Y, p_egalax_i2c->downCnt);
//printk(" X=%d Y=%d \n", PointBuf[ContactID].X, PointBuf[ContactID].Y);
	...
	if( bNeedReport || (ContactID <= LastUpdateID) )
	{
		...
		input_sync(input_dev);
	#endif
//		printk("[Egalax_i2c] Input sync point data done! (Down:%d Up:%d)\n", cnt_down, cnt_up);
	}
...
//printk("[Egalax_i2c]%s\n", __FUNCTION__);
	...
	if(pInputDev == NULL)
	{
		printk("[Egalax_i2c] Failed to allocate input device\n");
		return NULL;//-ENOMEM;
	}
...
//	input_set_abs_params(pInputDev, ABS_MT_POSITION_Y, 0, 2047, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_POSITION_X, 0, 800, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_POSITION_Y, 0, 1280, 0, 0);
	...
	if(ret) 
	{
		printk("[Egalax_i2c] Unable to register input device.\n");
...
//	printk("[Egalax_i2c] egalax_i2c_measure\n");
	...
	{
		printk("[Egalax_i2c] I2C read error data with Len=%d hedaer=%d\n", count, x_buf[0]);
		return -1;
	}

//	printk("[Egalax_i2c] I2C read data with Len=%d\n", count);
	if(x_buf[0]==REPORTID_VENDOR)
	{
		atomic_set(&wait_command_ack, 1);
		printk("[Egalax_i2c] I2C get vendor command packet\n");
		...
		if(egalax_i2c->TPType == TYPE_NONE)
		{
			...
			printk("[Egalax_i2c] Get Device type=%d\n", egalax_i2c->TPType);
		}
	}
	...
	if( count>0 && p_char_dev->OpenCnts>0 ) // If someone reading now! put the data into the buffer!
	{
		...
		if(ret>0) // fifo size is ready
		{
			...
			printk("[Egalax_i2c] [Warning] Can't write data because fifo size is overflow.\n");
	}
...
//	printk("[Egalax_i2c] egalax_i2c_wq run\n");
...
//	printk("[Egalax_i2c] egalax_i2c_wq leave\n");
}
...
//	printk("[Egalax_i2c] INT with irq:%d\n", irq);
...
static void egalax_i2c_wq_idle(struct work_struct *work)
{
	...
	if(!egalax_i2c)
	{
		printk("[Egalax_i2c] %s return with pointer error:%p\n", __func__, egalax_i2c);
		return; 
	}
	...
	if(egalax_i2c->work_state == MODE_WORKING)
	{
		...
		if(ret==MAX_I2C_LEN)
		{
			...
			printk("[Egalax_i2c] Set controller to idle mode\n");
		}
		else
			printk("[Egalax_i2c] Try to set controller to idle failed:%d\n", ret);
	}
}
...
static void egalax_i2c_early_suspend(struct early_suspend *handler)
{
	...
	printk("[Egalax_i2c] Enter early_suspend state:%d\n", p_egalax_i2c_dev->work_state);
	...
	if(p_egalax_i2c_dev->work_state == MODE_IDLE)
	{
		printk("[Egalax_i2c] Wakeup controller from idle mode before entering suspend\n");
		...
		printk("[Egalax_i2c] Device return to working\n");
	}
#endif
	...
	printk("[Egalax_i2c] Early_suspend done!!\n");
	return;

fail_suspend2:
	p_egalax_i2c_dev->work_state = MODE_SUSPEND;
fail_suspend:
	printk("[Egalax_i2c] Early_suspend failed!!\n");
	return;
}
...
static void egalax_i2c_early_resume(struct early_suspend *handler)
{
	...
	printk("[Egalax_i2c] Enter early_resume state:%d\n", p_egalax_i2c_dev->work_state);
...
//	gpio = irq_to_gpio(p_egalax_i2c_dev->client->irq);
	...
	printk("[Egalax_i2c] Early_resume done!!\n");
	return;

fail_resume2:
	p_egalax_i2c_dev->work_state = MODE_WORKING;
fail_resume:
	printk("[Egalax_i2c] Early_resume failed!!\n");
	return;
}
...
//	u8 cmdbuf[MAX_I2C_LEN]={0x03, 0x03, 0x0A, 0x01, 0x46, 0, 0, 0, 0, 0};
	u8 cmdbuf[MAX_I2C_LEN]={0x03, 0x05, 0x0A, 0x03, 0x36, 0x3f, 0x01, 0, 0, 0};
...
//printk("egalax_i2c_wq_loopback\n");

	if(atomic_read(&wait_command_ack) || egalax_i2c->work_state!=MODE_WORKING)
	{
		printk("[Egalax_i2c] Close egalax_i2c_wq_loopback work\n");
		return;
	}
...
#if 0
//[*]--------------------------------------------------------------------------------------------------[*]
static void		egalax_touch_reset(void)
{
//TSP_INT:jhkang	
...
#if 0
/* * I2C client */
static int egalax_detect(struct i2c_client *client, struct i2c_board_info *info)
{	
	int id;	
...
//#endif
	...
	if (!p_egalax_i2c_dev) 
	{
		printk("[Egalax_i2c]  Request memory failed\n");
		ret = -ENOMEM;
		goto fail1;
	}
	...
	if(input_dev==NULL)
	{
		printk("[Egalax_i2c] allocate_Input_Dev failed\n");
		ret = -EINVAL; 
		goto fail2;
	}
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[Egalax_i2c] Egalax_I2C check failed\n");
		ret = -ENODEV;
		goto fail2;
	}
	//i2c_set_clientdata(client, p_egalax_i2c_dev);  //jhkang
#endif
	...
	if( ret ) 
	{
		printk("[Egalax_i2c] Request irq(%d) failed\n", client->irq);
		goto fail3;
	}
...
{
	u8 cmdbuf[MAX_I2C_LEN]={0x03, 0x03, 0x0A, 0x01, 'D', 0, 0, 0, 0, 0};
...
{
	u8 cmdbuf[MAX_I2C_LEN]={0x03, 0x03, 0x0A, 0x01, 'E', 0, 0, 0, 0, 0};
...
while(gpio_get_value(gpio) );		
	...
	printk("[Egalax_i2c] Register early_suspend done\n");
#endif
#endif
...
//wakeup_controller(client->irq);
	...
	printk("[Egalax_i2c] I2C probe done\n");
	return 0;
...
fail1:
	...
	printk("[Egalax_i2c] I2C probe failed\n");
	return ret;
}
...
static int __devexit egalax_i2c_remove(struct i2c_client *client)
{
	...
	if(input_dev)
	{
		printk("[Egalax_i2c] Unregister input device\n");
		input_unregister_device(input_dev);
		input_dev = NULL;
	}
...
static void egalax_i2c_ts_exit(void)
{
	...
	if(p_char_dev)
	{
		printk("[Egalax_i2c] Unregister character device\n");
	...
	printk("[Egalax_i2c] Exit driver done!\n");
}
...
static struct egalax_char_dev* setup_chardev(dev_t dev)
{
	...
	if(result)
	{
		printk("[Egalax_i2c] Failed at cdev added\n");
		goto fail_kfifo;
	}
...
static int egalax_i2c_ts_init(void)
{
	...
	printk("--------------------------------------------------------\n");
	printk("\n%s(%d) : egalax init !!(Wake_lock_init)\n\n", __FUNCTION__, __LINE__);
	printk("--------------------------------------------------------\n");
	...
	if (result < 0)
	{
		printk("[Egalax_i2c] Cdev can't get major number\n");
		return 0;
	}
	...
	if(IS_ERR(egalax_class))
	{
		printk("[Egalax_i2c] Failed in creating class.\n");
		result = -EFAULT;
		goto fail;
	}
...
#endif
	printk("[Egalax_i2c] Register egalax_i2c cdev, major: %d \n",global_major);

	dbgProcFile = create_proc_entry(PROC_FS_NAME, 0666, NULL);
	if (dbgProcFile == NULL) 
	{
		remove_proc_entry(PROC_FS_NAME, NULL);
		printk("[Egalax_i2c] Could not initialize /proc/%s\n", PROC_FS_NAME);
	}
	else
	{
		dbgProcFile->read_proc = egalax_proc_read;
		dbgProcFile->write_proc = egalax_proc_write;
		printk("[Egalax_i2c] /proc/%s created\n", PROC_FS_NAME);
	}

	printk("[Egalax_i2c] Driver init done!\n");
...
//subsys_initcall(egalax_i2c_ts_init);
