...
void bcm_wlan_power_on(int flag)
{
	if (flag == 1) {
		...
		sdhci_s3c_force_presence_change(&s3c_device_hsmmc0, 1);
...
void bcm_wlan_power_off(int flag)
{
	if (flag == 1) {
		...
		sdhci_s3c_force_presence_change(&s3c_device_hsmmc0, 0);
